...
class ProjectViewer(ft.Column):
    def __init__(self, project_path: str):
        ...
        self.title = ft.Text(
            value="PROJECTS", color="#f0f0f0", size=20, weight=ft.FontWeight.BOLD
        )
    ...
    def _build_title(self):
        return ft.Container(
            content=ft.Row(
                controls=[self.title], alignment=ft.MainAxisAlignment.CENTER
            ),
            bgcolor="#909090",
            padding=10,
        )
    ...
    def _build_project_list(self):
        return ft.Container(content=self.project_list, bgcolor="#f0f0f0", expand=1)
    ...
    def refresh(self):
        ...
        if not self.project_path or not self.project_path.exists():
            ...
            for project in self.project_path.iterdir():
                if project.is_dir() and project.name != ".venv":
    ...
    def _on_project_click(self, project: Path):
        ...
        if os_system == "Windows":
            ...
            raise OSError(f"Unsupported operating system: {os_system}")
